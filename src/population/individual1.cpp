...
individual1::individual1(int windowWidth, int windowHeight, int circlesNumber)
	:individual(windowWidth, windowHeight, circlesNumber){
	
	individual::setGeneSetLength(individual::getXGenes() + individual::getYGenes() + 8 + 8 + 8 + individual::getMaxRadiusBinary());
	individual::setChromosomeLength((individual::getXGenes() + individual::getYGenes() + 8 + 8 + 8 + individual::getMaxRadiusBinary()) * circlesNumber);
	for(int i = 0; i < individual::getChromosomeLength(); ++i){//r+g+b+radius
		if(i % individual::getGeneSetLength() != 0){
			...
			//std::cout<<xBinary.size()<<"\t"<<individual::getXGenes()<<"\t"<<yBinary.size()<<"\t"<<individual::getYGenes()<<std::endl;
			//if(xBinary.size() != individual::getXGenes() || yBinary.size() != individual::getYGenes()){
	...
	//std::cout<<"chromosomeLength: "<<individual::getChromosomeLength()<<std::endl;

	individual::setFitness(0);
	updateIndividualsImage();
}

individual1::individual1(int windowWidth, int windowHeight, int circlesNumber, std::vector<int> genotype)
	:individual(windowWidth, windowHeight, circlesNumber){
	...
	//std::cout<<"circles Number: "<<genotype.size()<<std::endl;
}

std::vector<std::vector<int>> individual1::getCirclesData(){
	...
	while(i < genotype.size()){
		...
		for(int j=i+individual::getXGenes()+individual::getYGenes()+8+8+8; j<i+individual::getXGenes()+individual::getYGenes()+8+8+8+individual::getMaxRadiusBinary(); j++){
			pRadius.push_back(genotype[j]);
		}

		i = i + individual::getXGenes()+individual::getYGenes()+8+8+8+individual::getMaxRadiusBinary();
...
void individual1::updateIndividualsImage(){
